...
def digitize_coefficients(coefficients, bit_width=16):
    """Digitize the filter coefficients to fixed-point representation without using int32."""
    scale_factor = 2 ** (bit_width - 1)
    return [int(coeff * scale_factor) for coeff in coefficients]
...
def apply_filter():
    try:
        ...
        sweep_signal = (sweep_signal * 2 ** (output_width - 1)).astype(int)
        ...
        b_digital = digitize_coefficients(b, output_width)
        a_digital = digitize_coefficients(a, output_width)
